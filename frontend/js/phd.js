...
async function loadSidebarFilters() {
    ...
    try {
        const [colleges, resourceTypes] = await Promise.all([
            window.api.get('/meta/colleges').catch(() => []),
            window.api.get('/meta/resource-types?program=PhD').catch(() => [])
        ]);
...
async function loadResources() {
    ...
    container.innerHTML = `
        <div class="py-12 text-center text-slate-400">
            ...
            <p class="text-xs">Loading approved PhD thesis, research proposals & datasets...</p>
        </div>
    `;

    try {
        let endpoint = `/resources?program=PhD&sort=${currentFilters.sort}&page=${currentFilters.page}&limit=15`;
...
function renderResources(resources, container) {
    if (!resources || resources.length === 0) {
        container.innerHTML = `
            <div class="text-center py-12 text-slate-500 bg-white rounded-xl border border-dashed border-slate-300">
                <p class="text-sm font-semibold text-slate-700">No PhD research materials found</p>
    ...
    resources.forEach(res => {
        ...
        let typeBadgeColor = 'bg-indigo-600';
        ...
        card.innerHTML = `
            <div class="flex items-start gap-3 min-w-0 flex-1">
                ...
                <div class="space-y-1 min-w-0 flex-1">
                    <h3 class="text-xs font-bold text-slate-900 hover:text-indigo-600 cursor-pointer transition break-words" onclick="window.location.href='/resource.html?id=${res.id}'">
                        ${res.title}
                    </h3>
                    ...
                    <div class="flex flex-wrap items-center gap-1.5 pt-0.5">
                        <span class="px-2 py-0.5 rounded text-[10px] bg-indigo-50 text-indigo-700 font-medium">${res.resource_type_name || 'PhD Thesis'}</span>
                        <span class="px-2 py-0.5 rounded text-[10px] bg-blue-50 text-blue-700 font-medium">${res.subject_name || res.semester_name || 'Research'}</span>
            ...
            <div class="flex flex-wrap items-center justify-between sm:justify-end gap-2 sm:gap-5 shrink-0 text-xs text-slate-500 pt-2 sm:pt-0 border-t sm:border-t-0 border-slate-100">
                <div class="flex items-center gap-2">
                    ...
                    <div>
                        <span class="block font-semibold text-slate-800 text-[11px] leading-none">${res.contributor_name || 'PhD Researcher'}</span>
                        <span class="block text-[10px] text-slate-400">${res.college_name || 'Research Institution'}</span>
                    </div>
                </div>
                ...
                <button class="text-slate-400 hover:text-indigo-600 transition p-1" onclick="toggleBookmark(${res.id}, this)" title="Save to Vault">
...
async function toggleBookmark(id, btnElement) {
    ...
    try {
        ...
        if (data.bookmarked) {
            ...
            btnElement.classList.add('text-indigo-600', 'fill-current');
            showToast('Saved to My Vault!');
        } else {
            btnElement.classList.add('text-slate-400');
            btnElement.classList.remove('text-indigo-600', 'fill-current');
